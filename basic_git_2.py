...
class BasicGit:
    ...
    def _hash_object(self, data: str, obj_type: str = "blob") -> str:
        ...
        encoded_data = data.encode("utf-8")  # convert the text to computer-friendly format # YOUR CODE HERE
        ...
        header = f"{obj_type} {len(encoded_data)}\0".encode("utf-8") # YOUR CODE HERE
        if header is None:
            raise NotImplementedError(
        ...
        store = header + encoded_data # YOUR CODE HERE
        if store is None:
            raise NotImplementedError(
        ...
        hash_object = hashlib.sha1(store) # YOUR CODE HERE
        hex_hash = hash_object.hexdigest()  # create a unique fingerprint for this content # YOUR CODE HERE
        if hex_hash is None:
            raise NotImplementedError(
    ...
    def status(self) -> None:
        ...
        try:
            with open(self.index_file, "r") as f: # YOUR CODE HERE
                staged_files = [line.strip() for line in f if line.strip()] # YOUR CODE HERE
        except FileNotFoundError:
            staged_files = []

        print("Changes to be committed:")
        if staged_files:
            for file in staged_files: # YOUR CODE HERE
                print(f"  staged:   {file}")
        else:
